@if ($crud->hasAccess('update') && ((isset($crud->updateCondition) && call_user_func_array($crud->updateCondition, array($entry))) || !isset($crud->updateCondition)))
	@if (!$crud->model->translationEnabled())

	<!-- Single edit button -->
	<button onclick="window.location='{{ url($crud->route.'/'.$entry->getKey().'/edit') }}'" class="btn btn-sm btn-link"><i class="la la-edit"></i> {{ trans('backpack::crud.edit') }}</button>
