...
class ReportUserExport implements FromView, WithEvents, WithDrawings
{
    public function __construct($entries = [])
    {
        ...
        'Level Name', 'Head of Department', 'GoA', 'Cost Center', 'Active', 'Remark'];
    }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class    => function(AfterSheet $event) {
                ...
                $end = 'O'; // based on notes so, it should be BE
    ...
    public function view(): View
    {
        ...
        if (isset($paramUrl['level_id'])) {
            $users->where('mst_users.level_id', (int)$paramUrl['level_id']);
        }
        if (isset($paramUrl['department_id'])) {
            $users->where('mst_users.department_id', (int)$paramUrl['department_id']);
        }
        if (isset($paramUrl['goa_holder_id'])) {
            $users->where('mst_users.goa_holder_id', (int)$paramUrl['goa_holder_id']);
        }
        if (isset($paramUrl['cost_center_id'])) {
            $users->where('mst_users.cost_center_id', (int)$paramUrl['cost_center_id']);
        }

        $users = $users->get(['mst_users.user_id', 'vendor_number', 'mst_users.name', 'email', 'bpid', 'bpcscode', 'roles', 
                'mst_levels.level_id as lvl_code', 'mst_levels.name as lvl_name', 'mst_departments.name as md_name', 
